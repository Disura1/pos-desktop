...
import { getBranchComparison, getDailySummary, getRevenueByPeriod, getTopProducts } from '../../services/reportService';
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%' }}>
            <div title={fmtCurrency(d.revenue)} style={{ width: '100%', height: h, background: 'var(--pink)', borderRadius: '3px 3px 0 0', opacity: 0.85 }} />
...
const OwnerDashboard = () => {
  ...
  const totalRevenue7d = chartData.reduce((s, d) => s + parseFloat(d.revenue), 0);
  ...
  return (
    <div className="page-content">
      ...
      <div className="grid-2" style={{ gap: 20, marginBottom: 20 }}>
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-sub)', marginTop: 2 }}>Total: {fmtCurrency(totalRevenue7d)}</div>
            </div>
          </div>
        ...
        <div className="card">
          <div className="card-header"><div className="card-title">Branch Performance (Today)</div></div>
          {branches.length === 0 ? (
            <div className="empty-state"><span className="empty-state-icon">🏪</span><div className="empty-state-text">No branch data</div></div>
          ) : branches.map(b => (
            <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 13 }}>{b.branch_name}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{b.today_transactions || 0} transactions</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--pink)' }}>{fmtCurrency(b.today_revenue || 0)}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>30d: {fmtCurrency(b.month_revenue || 0)}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="grid-2" style={{ gap: 20 }}>
        <div className="card">
          <div className="card-header"><div className="card-title">Top Products (30 Days)</div></div>
                ...
                {topProducts.map((p, i) => (
                  <tr key={i}>
                    <td>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{p.sku} {p.size && `· ${p.size}`} {p.color && `· ${p.color}`}</div>
                    </td>
                    <td><span className="badge badge-info">{p.total_sold}</span></td>
                    <td style={{ fontWeight: 600 }}>{fmtCurrency(p.total_revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          <div className="card-header"><div className="card-title">⚠️ Low Stock Alerts</div></div>
          {lowStock.length === 0 ? (
            <div className="empty-state"><span className="empty-state-icon">✅</span><div className="empty-state-text">All stocks are healthy</div></div>
          ) : (
            <table>
              <thead><tr><th>Product</th><th>Branch</th><th>Stock</th></tr></thead>
              <tbody>
                {lowStock.slice(0, 8).map((ls, i) => (
                  <tr key={i}>
                    <td>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{ls.product_name}</div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{ls.sku} {ls.size && `· ${ls.size}`}</div>
                    </td>
